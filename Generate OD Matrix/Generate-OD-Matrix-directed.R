...
#Firstly we obtain the path where "Generate-OD-matrix-directed.R" is located
path<-print(rstudioapi::getActiveDocumentContext()$path)
folder<-gsub("Generate-OD-Matrix-directed.R","",path)
...
#case of undirected matrix add following line:
#M[columna,fila]=M[columna,fila]+1
}

write.csv(M, file = paste(folder,"OD_Matrix_directed.csv"))